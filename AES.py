...
def _mix_columns(state):
    ...
    for i, col in enumerate(state):
        ...
        s.append(b * col[0] + c * col[1] + col[2] + col[3])
...
def _add_round_key(state, key):
    for i in range(4):
        state[i] = [state[i][j] ^ key[i][j] for j in range(4)]
...
def hexstr(bytelist):
    return " ".join(hex(byte)[2:].zfill(2) for byte in bytelist)
...
class AES:
    ...
    def _key_schedule(self, key):
        ...
        for i in range(self._nk, 4 * (self._nr + 1)):
            ...
            if i % self._nk == 0:
                ...
                tmp[0] ^= RCON[i // self._nk]
            elif self._nk == 8 and i % self._nk == 4:
                tmp = _sub_word(tmp)
            w.append([x ^ y for x, y in zip(w[i - self._nk], tmp)])

        return w

    def _encrypt_block(self, text, w):
        state = list()
        for i in range(4):
            state.append(text[4*i:4*(i+1)])
    ...
    def encrypt(self, text, key):
        ...
        n = (len(text) - 1) // 16
        text = text + [0] * (16*(n + 1) - len(text))
        cipher = []
        w = self._key_schedule(key)
        while text:
            cipher.extend(self._encrypt_block(text[:16], w))
            text = text[16:]
